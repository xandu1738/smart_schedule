...
import { useEditDepartmentMutation } from "../../helpers/redux/slices/extendedApis/departmentsApi";

const EditDepartment = ({ setShowDialog, setRefetch, department }) => {
  const [editDepartment, { isLoading }] = useEditDepartmentMutation();

  // Debug log to check if department data is received
  console.log("EditDepartment received department:", department);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      editDepartment(values)
        .then((res) => {
          console.log("Department edited successfully:", res);
        })
        .catch((err) => {
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-4">
            <div className="flex flex-col justify-center items-center">
              ...
              <Button
                ...
                buttonName={isLoading ? <Spinner /> : "Edit"}
                disabled={isLoading}
              />
            </div>
          </form>
